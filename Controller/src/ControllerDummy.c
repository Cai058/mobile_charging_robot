		 ...
		 { 
			 ...
			 else
			 {
				...
				else
				{
					...
					if(slow_cnt > 5500)
					{
							(m_ctrl.move_direction == 1) ? SetxSpeed(-slow_speed_2nd) : SetxSpeed(slow_speed_2nd);
					}
					else
					{
						  slow_cnt ++;
...
int get_direction(uint8_t  location_id, uint8_t  target_id) {
	...
	//return (location_id <= target_id) ? 1 : -1;
}
